...
function toRequest(req: VercelRequest): Request {
  ...
  return new Request(new URL(req.url ?? "/api/admin/generate", origin), {
    method: req.method,
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  const response = await handleGenerate({ request: toRequest(req), storage, adminSecret: process.env.ADMIN_SECRET });
